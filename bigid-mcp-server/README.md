...
| `BIGID_LAZY_SCHEMAS` | Enable lazy input schema advertising (truncate to top-level, strip examples) | No | `true` |
| `BIGID_HIDE_OUTPUT_SCHEMAS` | Hide output schemas from advertised tool list (fetch on demand) | No | `true` |

### Lazy loading and schema expansion

- By default, input schemas are truncated to top-level depth to reduce context size, and examples are stripped. Use the always-available tool `expand_schema` to fetch deeper schema nodes on demand.
- Output schemas are hidden by default. Retrieve them on demand with `expand_schema` by setting `schemaType: "output"`. Set `BIGID_HIDE_OUTPUT_SCHEMAS=false` to include them in the advertised tool list.
- To disable lazy input truncation entirely, set `BIGID_LAZY_SCHEMAS=false`.

Example usage of `expand_schema`:

```json
{
  "name": "expand_schema",
  "arguments": {
    "toolName": "get_catalog_objects",
    "path": "/properties/structuredFilter"
  }
}
```

Fetch full output schema for a tool:
